...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	supertechnician: {
		...
		name: "Super Technician",
		rating: 5,
		num: -501,
	},
	magneticarmor: {
		shortDesc: "Halves the damage taken from physical moves and prevents critical hits.",
		onSourceModifyDamage(damage, source, target, move) {
		  if (move.category === 'Physical') {
			return this.chainModify(0.5);
		  }
	  ...
	  mistyaura: {
		onTryBoost(boost, target, source, effect) {
			...
			if (showMsg && !(effect as ActiveMove).secondaries && effect.id !== 'octolock') {
				this.add("-fail", target, "unboost", "[from] ability: Misty Aura", "[of] " + target);
			}
		},
		isBreakable: true,
		name: "Misty Aura",
	...
	dracorevive: {
		name: "DracoRevive",
		desc: "Once per battle, when the user faints, it is revived with full HP.",
		shortDesc: "Once per battle, user revives with full HP upon fainting.",
		onStart(pokemon) {
		  pokemon.side.addSideCondition('dracorevive', pokemon);
		},
		effectType: "Ability",
		// Additional data to handle the revive effect
		flags: {},
		condition: {
		  duration: 1,
		  onStart(target) {
			this.add('-ability', target, 'DracoRevive');
		  },
		  onFaint(target) {
			if (!target.side.getSideCondition('dracorevive') || target.side.getSideCondition('dracorevive').counter > 1) return;
			const revivePokemon = this.dex.deepClone(target);
			target.side.removeSideCondition('dracorevive');
			revivePokemon.hp = Math.floor(revivePokemon.maxhp / 2);
			revivePokemon.boosts.spa = 2; // 50% boost to Special Attack (equivalent to +1 stage)
			this.add('-ability', target, 'DracoRevive');
			this.add('-revive', revivePokemon, target);
			revivePokemon.side = target.side;
			revivePokemon.setPosition(target.position, true);
			target.side.pokemon[target.position] = revivePokemon;
		  },
		rating: 4,
		num: -505,
		},
	...
	dracoshield: {
		name: "Dracoshield",
		desc: "Reduces damage taken by attacks by 50% with a 50% chance.",
		shortDesc: "50% chance to reduce damage taken by 50%.",
		onSourceModifyDamage(damage, source, target, move) {
		  if (this.randomChance(1, 2)) {
			this.add("-ability", source, "Dracoshield");
			return this.chainModify(0.5);
		  }
		},
		rating: 4,
	...
	roughsurge: {
		onStart(source) {
			this.field.setTerrain('roughterrain');
		},
		name: "Rough Surge",
		rating: 4,
		num: -504,
	...
	fierysurge: {
		onStart(source) {
			this.field.setTerrain('feiryterrain');
		},
		name: "Fiery Surge",
		rating: 4,
		num: -507,
